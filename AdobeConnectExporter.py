...
Developed by Franck Dernoncourt. Edited and customized by Parsa Hejabi.
'''
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Download an Adobe Connect recording and convert to a video file.')
    parser.add_argument('URLorIDorZIP', nargs='*',
                        help='URL, code, or ZIP file for the Connect recording')
    parser.add_argument(
        '--output_folder', default='output_videos', help='Folder for output files')
    ...
    with open('cookie.txt', 'r') as file:
        cookie_header = file.read().replace('\n', '')
    ...
    unzip_command = 'unzip -n {0} -d {1}'.format(
        output_zip_filename, output_folder)
